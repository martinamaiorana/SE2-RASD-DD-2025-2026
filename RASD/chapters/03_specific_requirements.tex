...
\begin{itemize}
    ...
    \item \textbf{Trips History Page}: The system provides a "Your history" interface that displays a chronological list of recorded activities. Each entry includes the general location (e.g., Milano or Polimi), the date of the trip, and a validation status tag such as "To validate" or "Validated."
    ...
    \item \textbf{Path Viewing Page}: Upon searching, the system displays "Available Paths" (labeled Path A, B, and C) on an interactive map. A list at the bottom summarizes each path by its travel time and condition (e.g., "Optimal" or "Requires Maintenance"), with the most efficient route highlighted as "Recommended."

    \item \textbf{Trip Viewing (Trip Summary) Page}: The "Trip summary" page provides a detailed overview of a recorded session. It displays key performance data including Total Time, Total distance (e.g., 1.3km), Average Speed (e.g., $21~km/h$), and the Current Weather conditions (e.g., Sunny, $22^{\circ}C$).

    \item \textbf{Trip Deletion Page}: The system allows users to remove trips from their personal history. When a deletion is initiated, an "Are you sure?" prompt appears, warning that the action is irreversible. A "Trip deleted successfully!" notification confirms the removal of the record from the user's history.
\end{itemize}

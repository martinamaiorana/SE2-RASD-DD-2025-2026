...
\begin{itemize}
    \item \textbf{[R5]}: The system must allow a Registered User to automatically record a trip, using device sensors
    \item \textbf{[R6]}: The system must automatically pause recording when the user stops cycling and resume when cycling activity is detected again.
    \item \textbf{[R7]}: The system must automatically start GPS-based tracking when biking is detected.
    \item \textbf{[R8]}: The system must allow the Registered User to start the recording process by pressing a button
    \item \textbf{[R9]}: The system must collect accelerometer and gyroscope data to detect potential anomalies (e.g., potholes).
    \item \textbf{[R10]}: The system must store complete trip statistics (distance, average speed, duration and weather conditions at the time of recording).
    \item \textbf{[R11]}: The system must allow a registered user to view one of his automatically recorded trips enriched with statistics such as distance, average speed, duration and weather conditions at the time of recording
    ...
    \item \textbf{[R13]}: The system must allow the Registered User to stop the recording process by pressing a button
...
\begin{itemize}
    ...
    \item \textbf{[R18]}: The system must allow a Registered User to review the statistics related to a bike trip, such as average speed, and other performance metrics.
\end{itemize}

\subsubsection{Path Search, Routing and Visualization}

\begin{itemize}
    \item \textbf{[R19]}: The system must allow any user to view paths
    ...
    \item \textbf{[R22]}: The system must compute a Path Score based on Path status and estimated trip time (effectiveness).
    \item \textbf{[R23]}: The system must rank available paths, with same endpoints, based on their Path Score.
    \item \textbf{[R24]}: The system must allow a User to inspect public details of a selected path, such as status, total distance, average trip time, and merged anomalies.
    \item \textbf{[R25]}: The system shall query an external weather service to retrieve current weather conditions during the path visualization.
\end{itemize}

\subsubsection{Merging \& Freshness-Based Status Aggregation}

\begin{itemize}
    \item \textbf{[R26]}: The system shall append historical weather data (temperature, wind speed and direction) to the automatic trips of registered users for performance analysis.
    ...
    \item \textbf{[R28]}: The system must aggregate confirmed obstacles from published trips into their corresponding public path during the merge process
    \item \textbf{[R29]}: The system must remove obstacles from a path when newer published trips covering the same trajectory do not report those obstacles, maintaining data consistency with current road conditions.
\end{itemize}
...
\textbf{Flow of events}       & 
\begin{enumerate}
    \item The \texttt{Registered User} selects a \texttt{private} trip from their history.
...
\hline
\multicolumn{3}{|p{14.5cm}|}{\centering \textbf{G1 - A registered user aims to record its cycling trips}} \\
\hline
\textbf{Use Cases} & \multicolumn{2}{c|}{\textbf{Requirements and Domain Assumptions}} \\
\hline
& \textbf{D.1.2} & It is assumed that the speed and acceleration of a user cycling are mathematically distinguishable from those of walking, running or driving. \\
\hline
...
\hline
UC4 & \textbf{R3} & The system must allow a Registered User to manually insert a bike path by listing the name and the status of each street comprising the path. \\
\hline
UC4 & \textbf{R4} & The system must store manually inserted paths in the User's private trip history. \\
\hline
UC3 & \textbf{R5} & The system must allow a Registered User to automatically record a trip, using device sensors. \\
\hline
UC3 & \textbf{R6} & The system must automatically pause recording when the user stops cycling and resume when cycling activity is detected again. \\
\hline
UC3 & \textbf{R7} & The system must automatically start GPS-based tracking when biking is detected. \\
\hline
UC3 & \textbf{R8} & The system must allow the Registered User to start the recording process by pressing a button. \\
\hline
UC3 & \textbf{R12} & The system must buffer trip data locally when network connectivity is unavailable and synchronize buffered trip data once connectivity is restored. \\
\hline
UC3 & \textbf{R13} & The system must allow the Registered User to stop the recording process by pressing a button. \\
\hline
\end{longtable}
...
\begin{longtable}{|p{2.5cm}|p{1cm}|p{10.5cm}|}
\hline
\multicolumn{3}{|p{14.5cm}|}{\centering \textbf{G2 - A registered user wants to access their previously recorded bike trips}} \\
\hline
\textbf{Use Cases} & \multicolumn{2}{c|}{\textbf{Requirements and Domain Assumptions}} \\
\hline
UC3, UC4, UC8 & \textbf{R16} & The system must allow Registered Users to store all their recorded trips (manually or automatically). \\
\hline
UC7, UC8 & \textbf{R17} & The system must allow a Registered User to view his private trip history, including relatively collected information. \\
\hline
\end{longtable}
...
\begin{longtable}{|p{2.5cm}|p{1cm}|p{10.5cm}|}
\hline
\multicolumn{3}{|p{14.5cm}|}{\centering \textbf{G3 - A registered user aims to receive statistics about its past trips (incl. weather)}} \\
\hline
\textbf{Use Cases} & \multicolumn{2}{c|}{\textbf{Requirements and Domain Assumptions}} \\
\hline
& \textbf{D.1.1} & It is assumed that GPS, accelerometer and gyroscope sensors are sufficiently precise to distinguish biking vibrations from road anomalies effectively. \\
\hline
UC7 & \textbf{R10} & The system must store complete trip statistics (distance, average speed, duration and weather conditions at the time of recording). \\
\hline
UC7 & \textbf{R11} & The system must allow a registered user to view one of his automatically recorded trips enriched with statistics such as distance, average speed, duration and weather conditions at the time of recording. \\
\hline
UC7 & \textbf{R26} & The system shall append historical weather data (temperature, wind speed and direction) to the automatic trips of registered users for performance analysis. \\
\hline
\end{longtable}
...
\begin{longtable}{|p{2.5cm}|p{1cm}|p{10.5cm}|}
\hline
\multicolumn{3}{|p{14.5cm}|}{\centering \textbf{G4 - A registered user wants to share information about a bike path to the community}} \\
\hline
\textbf{Use Cases} & \multicolumn{2}{c|}{\textbf{Requirements and Domain Assumptions}} \\
\hline
& \textbf{D.1.3} & It is assumed that Registered Users will generally provide truthful validations regarding obstacles and path status. \\
\hline
UC3 & \textbf{R9} & The system must collect accelerometer and gyroscope data to detect potential anomalies (e.g., potholes). \\
\hline
UC5 & \textbf{R14} & The system must allow a Registered User to confirm or reject each detected anomaly within 72 hours from the trip recording. \\
\hline
UC6 & \textbf{R15} & The system must allow a Registered User to publish a validated trip. \\
\hline
\end{longtable}
...
\begin{longtable}{|p{2.5cm}|p{1cm}|p{10.5cm}|}
\hline
\multicolumn{3}{|p{14.5cm}|}{\centering \textbf{G5 - A User wants to search for bike paths and visualize results on a map}} \\
\hline
\textbf{Use Cases} & \multicolumn{2}{c|}{\textbf{Requirements and Domain Assumptions}} \\
\hline
UC9, UC10 & \textbf{R19} & The system must allow any user to view paths. \\
\hline
UC9 & \textbf{R20} & The system must allow any User to search the bike path(s) between a given origin and a given destination. \\
\hline
UC9, UC10 & \textbf{R21} & The system must visualize the paths on a map interface. \\
\hline
\end{longtable}
...
\begin{longtable}{|p{2.5cm}|p{1cm}|p{10.5cm}|}
\hline
\multicolumn{3}{|p{14.5cm}|}{\centering \textbf{G6 - A User wants to retrieve up to date information about bike paths (status/weather)}} \\
\hline
\textbf{Use Cases} & \multicolumn{2}{c|}{\textbf{Requirements and Domain Assumptions}} \\
\hline
UC9 & \textbf{R22} & The system must compute a Path Score based on Path status and estimated trip time (effectiveness). \\
\hline
UC9 & \textbf{R23} & The system must rank available paths, with same endpoints, based on their Path Score. \\
\hline
UC10 & \textbf{R24} & The system must allow a User to inspect public details of a selected path, such as status, total distance, average trip time, and merged anomalies. \\
\hline
UC10 & \textbf{R25} & The system shall query an external weather service to retrieve current weather conditions during the path visualization. \\
\hline
UC6, UC10 & \textbf{R27} & The system must merge publishable information from multiple Users referring to the same path, considering their freshness and the number of confirming contributions. \\
\hline
UC6, UC10 & \textbf{R28} & The system must aggregate confirmed obstacles from published trips into their corresponding public path during the merge process. \\
\hline
UC6, UC10 & \textbf{R29} & The system must remove obstacles from a path when newer published trips covering the same trajectory do not report those obstacles, maintaining data consistency with current road conditions. \\
\hline
\end{longtable}
